...
void main() {
  ...
  vec3 view_dir = vec3(eye_pos-world_pos);
  // Calculate half vector between view_dir and light_dir
  vec3 half_vector = normalize(light_dir+view_dir);
  // Calculate k
  float kb = pow(max(dot(transformed_normal, half_vector), 0.0), shininess);
  // Calculate specular
  vec4 specular = kb * (specular_reflection*light_colour);
