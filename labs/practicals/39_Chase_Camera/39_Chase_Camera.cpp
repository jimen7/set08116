...
bool update(float delta_time) {
  ...
  if (glfwGetKey(renderer::get_window(), 'Q')) {
	  meshes["chaser"].get_transform().rotate(vec3(0, half_pi<float>(),0)*delta_time);
  }
  ...
  if (glfwGetKey(renderer::get_window(), 'W')) {
	  meshes["chaser"].get_transform().position.z -= 5.0f * delta_time;
  }
  if (glfwGetKey(renderer::get_window(), 'S')) { 
	  meshes["chaser"].get_transform().position.z += 5.0f * delta_time;
  }
  if (glfwGetKey(renderer::get_window(), 'A')) {
	  meshes["chaser"].get_transform().position.x -= 5.0f * delta_time;
  }
